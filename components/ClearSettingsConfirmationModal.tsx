...
interface ClearSettingsConfirmationModalProps {
  isOpen: boolean;
  onConfirm: () => void;
  onCancel: () => void;
  t: (key: string) => string;
...
export const ClearSettingsConfirmationModal: React.FC<ClearSettingsConfirmationModalProps> = ({ isOpen, onConfirm, onCancel, t }) => {
  if (!isOpen) {
    return null;
  }
  ...
  return (
    <div 
      ...
      aria-labelledby="clear-settings-modal-title" role="dialog" aria-modal="true"
      onClick={onCancel}
    >
      <div 
      ...
      >
        <div className="sm:flex sm:items-start">
          ...
          <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            <h3 className="text-lg leading-6 font-medium text-slate-100" id="clear-settings-modal-title">
              {t('clearSettingsModal.title')}
            </h3>
            <div className="mt-2">
              <p className="text-sm text-slate-400">
                {t('clearSettingsModal.message')}
              </p>
            </div>
          </div>
        </div>
